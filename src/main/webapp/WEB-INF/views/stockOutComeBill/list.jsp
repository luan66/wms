            ...
            <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
                ...
                <c:forEach items="${result.data}" var="list">
                    <tr>
                        ...
                        <c:if test="${list.status ==0}">
                            <td>
                                <a href="/stockOutComeBill/input.do?id=${list.id}">编辑</a>
                                <a href="javascript:;" data-id="${list.id}" class="btn_delete"
                                   data-url="/stockOutComeBill/delete.do?id="
                                >删除</a>
                                <a href="javascript:;" data-auditor="/stockOutComeBill/auditor.do?id=${list.id}"
                                   class="btn_auditor"
                                >审核</a>
                            </td>
                        </c:if>
                        <c:if test="${list.status ==1}">
                            <td>
                                <a href="/stockOutComeBill/view.do?id=${list.id}">查看</a>
                            </td>
                        </c:if>
                    </tr>
                </c:forEach>
